  ...
  return (
    <footer className="w-full mx-auto bottom-0 bg-[#1d1d1f] px-8 lg:px-0">
      <div className="md:w-[692px] lg:w-[700px] mx-auto relative  flex flex-col items-center justify-center">
        ...
        <ol className="text-textGray px-0 py-4 footer-list list-decimal border-b border-b-textGray font-sf">
          {footnotes.map((footnote) => (
            <li key={footnote.id} id={footnote.id}
              dangerouslySetInnerHTML={{
                __html: (footnote.content || "").replace(
                  /<a /g, "<a class='text-white' "
                )
              }}
            />
          ))}
        ...
        <div className="bg-[#1D1D1F] text-textGray py-8 w-full">
          {/* Breadcrumb */}
          <nav className="mb-8 flex items-center gap-2 text-sm border-b border-b-textGray pb-4 md:pb-0">
            ...
            <a href="/mac" className="text-white hover:underline  opacity-75">
              Mac
            </a>
            <ChevronRight className="h-3 w-3 opacity-75" />
            <a href="/mac/macbook-pro" className="text-white hover:underline opacity-75">
              MacBook Pro
            </a>
          </nav>
            ...
            <Accordion type="single" collapsible className="w-full">
              {footerSections.map((section) => (
                <AccordionItem key={section.id} value={section.id} className="border-[#424245]">
                  <AccordionTrigger className="text-white text-sm font-medium opacity-75">{section.title}</AccordionTrigger>
                  <AccordionContent>
          ...
          <div className="hidden md:grid grid-cols-4 gap-8 mb-8">
            {footerSections.map((section) => (
              <div key={section.id}>
                <h3 className="text-white text-sm font-medium mb-4 opacity-75">{section.title}</h3>
          ...
          {/* Find a retailer */}
          <div className="mb-8 border-b border-b-textGray pt-4 md:pt-0 pb-4 ">
            <a href="/retail" className="text-[#2997FF] text-sm underline ">
              Find a retailer
            </a>
          ...
          {/* Footer Bottom */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center pb-4 text-xs border-b border-b-textGray">
