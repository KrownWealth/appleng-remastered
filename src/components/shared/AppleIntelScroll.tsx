  ...
  return (
    ...
    >
      ...
      <div className="relative pt-16 flex flex-col items-center justify-center">
        <ul className="flex gap-8 border-b border-gray-50">
          {slides.map((tab, index) => (
            <li
              ...
              className={`cursor-pointer relative px-1 transition-colors text-2xl ${currentIndex === index
        ...
        <p className="mt-4 text-lg text-center max-w-2xl text-textGray">{slides[currentIndex]?.description}</p>
      </div>
    </Carousel>
  );
}
